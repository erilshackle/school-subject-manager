...
void ConsoleDisplay(HANDLE* hStdout, SHORT COLS, SHORT LINES){
    ...
    SetConsoleTitle("GESTOR DE DISCIPLINAS");
        ...
        while( fscanf(ini, "%s %s", prop, vals) != EOF ){
            ...
            if( strcmp(prop, "theme:")==0 ){
                for(i = 0, fb = 0; i < menus_themes; i++){
                    ...
                    if(strcmp(CONFIG.numeric,"default")==0)     setlocale(LC_NUMERIC, "");
...
int erase_all_data(void){
    ...
    FILE* result;
    remove("sm-reset.vbs");
    if( (result = fopen("data/user.db", "r")) ){
        fclose(result);
        return 0;
    }
    else

...
#define DATAFOLDER_RUN system("if not exist data (mkdir data)");
#define DATAFOLDER_DEL system("if exist data (rd data /f)");

#define CONFIG_RUN configInit()
#define CONFIG_INI "data/config.ini"
#define CONFIG_DEL remove(CONFIG_DIR)
#define CONFIG_OPEN system("start notepad.exe data/config.ini")

/**< DISPLAY */

typedef enum CORES_DOS {Black,Blue, Green, Aqua, Red, Purple, Yellow, White,
            Gray,LBlue,LGreen,LAqua,LRed,LPurple,LYellow,BWhite}COLORS;
//! CONIO.H
